---
aliases: []
created: '2026-05-05'
crm_id: ''
crm_system: ''
first_seen: '2026-05-05'
founded: ''
founders: []
last_mention: '2026-05-11'
last_touch: ''
location: ''
mention_count: 3
name: Liquid AI
sector: ''
stage: ''
status: tracking
tags: []
type: company
website: ''
---

# Liquid AI

## Mentions



### 2026-05-11 — Link Ventures portfolio unicorn
<!-- source_hash: 2fc5582f5f47bf0eead34169433e7cf5741c3defeb2847122ca1a1abfd647524 -->
Fraser described Liquid AI as one of four AI unicorns in Link Ventures' current fund, characterizing it as a foundation model company. It was cited alongside [[Blitzie]] and two others as evidence of the fund's track record in applied AI.
↳ source: [[2026-05-11-pitch-call-vigil-ai-commerce-ad-platform-founders-v-badri]]

### 2026-04-26 — On-device model viability
<!-- source_hash: a12c9b0b0570a82c4263efeb61f86a0edda9e9cfd6c5c688123331b182c8f529 -->
During hackathon judging, one participant suggested the group look at Liquid AI's model as an example of a capable local foundation model that could run on a phone. The comment arose in the context of evaluating a robotics/lab-agent project that used a local model, with the implication that Liquid AI represented a credible benchmark for on-device inference quality.
↳ source: [[2026-04-26-mit-csail-hackathon-prize-deliberation-techtrek-program-update]]

### 2026-04-23 — Highest-valued MIT seed-stage company
<!-- source_hash: 12ec1a65d405a18eb8810a9e8da673587198866f28318f068590e4c821b73fff -->
John cited Liquid AI as the highest-valued company to emerge from MIT at the seed stage, with a valuation around $2.5 billion. The company made a significant announcement on the day of the session and maintains offices in both Boston and Silicon Valley, making it an example of the bi-coastal model John was encouraging Tech Trek participants to consider. John mentioned exploring whether Liquid AI might host participants.
↳ source: [[2026-04-23-tech-trek-summer-program-prospective-student-info-session]]