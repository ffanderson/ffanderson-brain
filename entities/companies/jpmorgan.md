---
aliases: []
created: '2026-05-09'
crm_id: ''
crm_system: ''
first_seen: '2026-05-09'
founded: ''
founders: []
last_mention: '2026-05-15'
last_touch: ''
location: ''
mention_count: 3
name: JPMorgan
sector: ''
stage: ''
status: tracking
tags: []
type: company
website: ''
---

# JPMorgan

## Mentions



### 2026-05-15 — Niels's finance background
<!-- source_hash: 0fd280b8c7b191a504f46a68624e20e853396d0909d7f618c8c6dba303bcdb0a -->
[[Niels]], co-founder of [[Polaris]], spent two years at JPMorgan as part of a finance career before joining Blackstone and subsequently enrolling at HBS to found a company.
↳ source: [[2026-05-15-startup-pitch-strategy-discussion-polaris-ai-native-medical-imaging-services]]

### 2026-05-07 — Build-vs-buy cautionary example
<!-- source_hash: dced6a7a7fb7e7a1dfd861c88845d48d723078feee2653bd858416b6274687f4 -->
Josh cited JPMorgan as an illustrative case of a large financial institution that publicly committed to building its own AI harnesses internally. His expectation, based on patterns observed at similar firms, was that they would encounter scaling and maintenance difficulties and eventually revert to third-party solutions, but that the cycle would take long enough to create a meaningful short-term headwind for startups selling into that space.
↳ source: [[2026-05-07-intro-call-chris-standard-kernel-and-josh-beacon-nadia]]

### 2026-05-09 — Paid POC benchmark pricing
<!-- source_hash: 43ef329822653aa56778b42f5fffb0185a99c8d2b0f1a23902e340d3ab53fd44 -->
Fraser cited [[JPMorgan]] as an example of the type of enterprise that should be charged meaningfully — on the order of a million dollars — for a proof-of-concept engagement, to reflect the engineering intensity and value delivered. This was used to illustrate the broader principle that paid pilots should take priority over unpaid ones.
↳ source: [[2026-05-09-gtm-strategy-pilot-prioritisation-chris-ai-kernel-optimisation-startup]]